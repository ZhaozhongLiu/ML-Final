    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for chosen_ids, chosen_mask, rejected_ids, rejected_mask in tqdm(train_loader, desc=f"DPO epoch {epoch}/{args.epochs}"):
            ...
            if now >= next_progress:
                elapsed = now - start
                print(f"[dpo] Progress: epoch={epoch}/{args.epochs} step={global_step} elapsed={elapsed:.0f}s last_loss={float(loss.item()):.4f}")
        ...
        if stop_early:
            ...
            print(f"[dpo] Reached {why}, stopping early at epoch={epoch}.")
            break
